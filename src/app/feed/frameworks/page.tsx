...
export default function Home() {
  ...
  const getFrameworksFetch = async () => {
    try {
      const url = new URL('https://backend-my-site.onrender.com/frameworks');
      ...
      if(responseData.status !== 0) {
        setFrameworks(responseData.data.frameworks)
  ...
  const removeFrameworkFetch = async (frameworkId: string) => {
    try {
      _modalContext.onOpenLoading()
      const response = await fetch(`https://backend-my-site.onrender.com/frameworks/${frameworkId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json', // Tipo de conteúdo que você está enviando
          'Authorization': `Bearer ${_userContext?.token}`,
  ...
  const editFramework = (frameworkId: string) => {
    router.push(`/feed/frameworks/editar/${frameworkId}`)
  }

  const showFramework = (href: string) => {
    router.push(href)
  }
    ...
    <main className="flex w-full p-4">
      <div className="grid grid-cols-1 w-full">
          ...
          <div className="flex items-center justify-between">
            <Breadcrumb spacing='4px' separator={<ChevronRightIcon w={5} h={5} color='black' />}>
              <BreadcrumbItem isCurrentPage>
                <BreadcrumbLink color="black">Frameworks</BreadcrumbLink>
              </BreadcrumbItem>
            </Breadcrumb>
        ...
        <div className="col-span-1 mt-4">
          <h1 className="mb-0 text-xl lg:text-2xl">Frameworks</h1>
        </div>

        <div className="col-span-1 mt-4 bg-white shadow-lg rounded p-3">
          <div className="flex">
            <Button onClick={() => goRouter('/feed/frameworks/criar')} colorScheme="green">
              Criar framework
            </Button>
          </div>
        </div>

        {
          ...
          <div className="col-span-1 mt-4 bg-white shadow-lg rounded p-3">
                      ...
                      <Popover placement='bottom-start'>
                        <PopoverTrigger>
                          <CardBody padding="8px">
                            <span className="">{framework.name}</span>
                          </CardBody>
                        </PopoverTrigger>
                        <PopoverContent maxW="180px">
                          <PopoverBody className="flex justify-center">
                            <Wrap>
                              ...
                              <IconButton
                                onClick={() => showFramework(framework.uri)}
                              ...
                              <IconButton
                                onClick={() => removeFrameworkFetch(framework.id)}
                              ...
                              <IconButton
                                onClick={() => editFramework(framework.id)}
            ...
            : <>
              <NotData title="Nenhum framework encontrado" />
            </>
            }      
          </div>
          : <>
