...
export default function Home() {
  ...
  const getJobsFetch = async () => {
    try {
      const url = new URL('https://backend-my-site.onrender.com/jobs');
  ...
  const removeJobFetch = async (jobId: string) => {
    try {
      _modalContext.onOpenLoading()
      const response = await fetch(`https://backend-my-site.onrender.com/jobs/${jobId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json', // Tipo de conteúdo que você está enviando
          'Authorization': `Bearer ${_userContext?.token}`,
  ...
  const editJob = (jobId: string) => {
    router.push(`/feed/trabalhos/editar/${jobId}`)
  }

  const showJob = (href: string) => {
    router.push(href)
  }
    ...
    <main className="flex w-full p-4">
      <div className="grid grid-cols-1 w-full">
          ...
          <div className="flex items-center justify-between">
            <Breadcrumb spacing='4px' separator={<ChevronRightIcon w={5} h={5} color='black' />}>
              <BreadcrumbItem isCurrentPage>
                <BreadcrumbLink color="black">Trabalhos</BreadcrumbLink>
              </BreadcrumbItem>
            </Breadcrumb>
        ...
        <div className="col-span-1 mt-4">
          <h1 className="mb-0 text-xl lg:text-2xl">Trabalhos</h1>
        </div>

        <div className="col-span-1 mt-4 bg-white shadow-lg rounded p-3">
          <div className="flex">
            <Button onClick={() => goRouter('/feed/trabalhos/criar')} colorScheme="green">
              Criar trabalho
            </Button>
          </div>
        </div>

        {
          ...
          <div className="col-span-1 mt-4 bg-white shadow-lg rounded p-3">
                      ...
                      <Popover placement='bottom-start'>
                        <PopoverTrigger>
                          <CardBody padding="8px">
                            ...
                            <span className="text-[0.8rem] font-[600]">{job.type}</span>
                            </div>
                          </CardBody>
                        </PopoverTrigger>
            ...
            : <>
              <NotData title="Nenhum trabalho encontrado" />
            </>
            }      
          </div>
          : <>
